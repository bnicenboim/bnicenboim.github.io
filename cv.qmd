...
inproceedings <- ReadBib("papers/papers_inproceedings.bib", check = FALSE)
presentations <- ReadBib("papers/selected.bib", check = FALSE)
...
sort(articles,sorting="ydnt")
sort(inproceedings,sorting="ydnt")
sort(inproceedings,sorting="ydnt")
sort(presentations,sorting="ydnt")
sort(underreview,sorting="ydnt")
sort(books,sorting="ydnt")
sort(unpublished,sorting="ydnt")
...
-   I was one of the organizers of the first [Computational Psycholinguistics Meeting (2025)](https://cpl2025.sites.uu.nl/): a new recurring meeting dedicated exclusively to computational psycholinguistics.
...
output(books, withbrackets = FALSE) 
```

## Published or accepted in peer-reviewed journals

```{r articles-cv,echo=FALSE,results="asis",cache=FALSE}
...
for(y in sort(unique(as.numeric(articles$year)), decreasing  = TRUE)){
   ...
   #     cat(paste0("## ",as.character(y),"\n" )) 
        output(articles_y, withbrackets=FALSE)  
    }
}    
```
...
## Unpublished/Under review manuscripts

```{r,echo=F,results="asis",cache=F}
 
output(underreview, withbrackets = FALSE) 
output(unpublished, withbrackets = FALSE) 
...
for(y in sort(unique(as.numeric(inproceedings$year)), decreasing  = TRUE)){
    ...
    inproceedings_y <- suppressMessages(inproceedings[list(year=as.character(y))])
        output(inproceedings_y,withbrackets=FALSE)
...
```{r,echo=F,results="asis",cache=F}
confs <- presentations[list(bibtype="inproceedings")]
for(y in sort(unique(as.numeric(confs$year)), decreasing  = TRUE)){

    confs_y <- suppressMessages(confs[list(year=as.character(y))])
        output(confs_y,withbrackets=FALSE)
...
talksall <- presentations[list(bibtype="misc")]

for(y in sort(unique(as.numeric(talksall$year)), decreasing  = TRUE)){

    talksall_y <- suppressMessages(talksall[list(year=as.character(y))])
        output(talksall_y,withbrackets=FALSE)
...
|  |  |
|:--------------------|:--------------------------------------------------|
| 2017-2025 | "Advanced Bayesian methods", |
...
|  |  |
|:--------------------|--------------------------------------------------:|
| 2025-2026 | **Lecturer** in "Research Workshop", BA in Cognitive Science & AI. Tilburg University |
| 2022-2025 | **Lecturer** in "Bayesian Multilevel Models/Bayesian Modeling for Data Science", Master in Data Science & Society. Tilburg University |
| 2021-2025 | **Lecturer** in "Bayesian Models of Cognitive Processes", Master in Cognitive Science & AI. Tilburg University |
