...
output <- function(articles, withbrackets=TRUE,hack=FALSE){
    for(i in 1:length(articles)){
        a <- capture.output(print(articles[i], .opts = list(no.print.fields="eprint") ))
              if(hack){
        ...
        if(withbrackets){
                ...
                brackets <- list(read= articles[i]$eprint,
